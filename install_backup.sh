...
# Archer - Arch Linux Home P}

# Main execution - simplified and robustransformation Suite
...
# Main execution - simplified and robust
main() {
    # Handle test mode
    if [[ "$1" == "--test" ]]; then
        export TEST_MODE="true"
        ...
        case "$choice" in
            ...
            2|3|4|5|6|7|8|9)
                if grep -q "archiso" /proc/cmdline 2>/dev/null && [[ "$EUID" -eq 0 ]] && [[ "$TEST_MODE" != "true" ]]; then
                    ...
                    echo -e "${CYAN}This will be available in a future update${NC}"
                fi
                read -p "Press Enter to continue..."
                ;;
            0)
