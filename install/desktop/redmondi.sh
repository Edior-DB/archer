...
main() {
    show_logo
    echo -e "\033[1;33mThis will switch your KDE Plasma layout to a Windows-like (Redmondi) layout using KDE's native API.\033[0m"
    ...
    # Use D-Bus to evaluate a Windows-like layout script from Archer configs (Plasma 6 compatible)
    ...
    if [ -f "$LAYOUT_PATH" ]; then
        ...
        echo -e "\033[32mRedmondi (Windows-like) layout applied using KDE API!\033[0m"
        ...
        echo -e "\033[33mPlease create a KDE Plasma layout script for Redmondi and place it in $LAYOUT_PATH.\033[0m"
    fi
}
