...
# Hyper Terminal Emulator Installation Script
# Electron-based terminal with modern web technologies
...
show_banner "Hyper Terminal Installation"

install_hyper() {
    echo -e "${BLUE}Installing Hyper...${NC}"

    # Install Hyper from AUR
    install_with_retries yay hyper
    ...
    if [[ ! -f ~/.hyper.js ]]; then
        echo -e "${YELLOW}Creating default Hyper configuration...${NC}"
        cat > ~/.hyper.js << 'EOF'
...
module.exports = {
  config: {
    ...
    fontFamily: '"JetBrains Mono", "DejaVu Sans Mono", Consolas, "Lucida Console", monospace',
...
EOF
        echo -e "${GREEN}Default configuration created at ~/.hyper.js${NC}"
    else
  ...
  if archer_confirm_or_default "Set Hyper as default terminal emulator?" "no"; then
        ...
        echo -e "${GREEN}Hyper set as default terminal${NC}"
    fi

    echo -e "${GREEN}Hyper installation completed!${NC}"
    echo -e "${CYAN}Configuration: ~/.hyper.js${NC}"
    echo -e "${CYAN}Launch with: hyper${NC}"
    echo -e "${CYAN}Plugin management: hyper install <plugin-name>${NC}"
}
